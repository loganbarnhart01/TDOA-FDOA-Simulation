...
def main():
    ...
    receiver1 = Receiver(21.80e6, 1e-6, np.array([100, 0, 0]))
    ...
    max_caf, tshift_caf, fshift_caf, caf_values = caf(noisy_signal1, noisy_signal2, 6, 800, 101)
    ...
    # X, Y = np.linspace(-800, 800, caf_values.shape[1]), np.linspace(-6, 6, caf_values.shape[0])
    # X, Y = np.meshgrid(X, Y)
